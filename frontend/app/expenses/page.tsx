...
export default function ExpensesPage() {
  ...
  const onSubmit = async (formData: FormData) => {
    setSubmitting(true);
    try {
      await entries.create({ ...formData, entry_type: "expense", payment_method: formData.payment_method as PaymentMethod });
    ...
    <AuthLayout>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">Track your spending</p>
        </div>
        <Button onClick={() => setShowForm(!showForm)}>
          <Plus className="h-4 w-4" /> {showForm ? "Cancel" : "Add Expense"}
        </Button>
      </div>

      {showForm && (
        <Card className="mb-6">
          <CardHeader><CardTitle>New Expense Entry</CardTitle></CardHeader>
          <CardContent>
        ...
        <CardContent className="p-0">
          {loading ? <PageLoading /> : data.length === 0 ? (
            <EmptyState
              ...
              description="Add your first expense entry to start tracking spending"
              action={<Button onClick={() => setShowForm(true)}><Plus className="h-4 w-4" /> Add Expense</Button>}
            />
          ) : (
            <Table>
              <THead>
              ...
              <TBody>
                {data.map((e) => (
                  <Tr key={e.id}>
                    ...
                    <Td><Badge variant="expense">{e.category}</Badge></Td>
                    ...
                    <Td className="text-right font-medium text-rose-600">-{formatCurrency(e.amount)}</Td>
                  </Tr>
                ))}
              </TBody>
            </Table>
